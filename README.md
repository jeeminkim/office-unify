...
Next.js 기반 모노레포입니다. **Dev Assistant(생성/요약)**, **Supabase 연동**(페르소나 채팅, 포트폴리오 원장, 투자위원회 등), **Google 로그인**을 사용합니다.

**대표 화면(참고):** `/persona-chat`, `/portfolio-ledger`, `/research-center`, `/trend`(트렌드 분석 센터), `/committee-discussion` 등 — 구현·계약은 `docs/` 및 `packages/ai-office-engine` 주석을 우선합니다.
