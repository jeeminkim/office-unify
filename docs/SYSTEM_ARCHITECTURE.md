...
- `/` : 개인 투자 대시보드
  ...
  - **Persona principles(EVO-034):** `apps/web/lib/personaPrinciples.ts` is the shared pure guardrail module for no-trade/no-auto-execution caveats, forbidden phrase detection, safe negated caveats, common section labels, and role snippets. Existing prompt/guard behavior remains intact; this layer centralizes copy and detection only.
  - **PB output contract audit(EVO-036):** `pbOutputContractValidator` adds warning-only quality summaries for PB message, PB Weekly, PB Daily Note preview, and Research send-to-PB. It reuses persona principles and never rewrites or blocks generated PB text.
...
- `/api/system/status`
  ...
  - additive: `sqlReadinessSummary`(요약만; 상세는 `/api/system/sql-readiness`)
...
- `/api/investor-profile`
  - `GET` 현재 사용자 프로필(없으면 기본값·`profileStatus: missing`)
  - `POST` upsert(허용 enum만, notes 길이 제한·sanitize)
- `/api/investor-profile/assess`
  - `POST` 적합성 미리보기(DB write 없음)
...
- 기존 투자 도구 API
  ...
  - **`/api/persona-chat/message`** 및 **`/api/persona-chat/message/stream`(NDJSON)** — 동일 서버 레이어에서 페르소나 구조화 산출물 계약 적용(`parsePersonaStructuredOutput` → validate → 금지 문구 sanitize → `insufficient_data` 폴백). 비스트림 JSON과 스트림 최종 `done` 모두 `personaStructuredOutput*`·`personaStructuredParseFailed` 등 additive 필드를 포함할 수 있다. 스트림 `done` 줄은 하위 호환을 위해 root에 `structuredOutput`·`structuredOutputSummary`·`parseFailed`·`fallbackApplied`·`bannedPhraseCount`·`personaWarnings`를 **중복**(내용은 `body`와 동일 계열) 제공할 수 있다. 중간 `delta` 청크는 모델 원문일 수 있으나 **저장되는 어시스턴트 메시지**와 **`done.body`**는 표시용 sanitize 후 문자열이다. **매수 추천·자동매매·자동 주문 없음.**
  ...
  - `/api/decision-retrospectives` — **GET** 목록(`status`/`symbol`/`sourceType`)·**`qualityMeta.decisionRetrospectives`**(건수·stale draft 30일+ 등); **read-only**(insert 미사용). **POST** 수동 생성(잘못된 `sourceType`/`status`/`outcome`/`qualitySignals`는 **400** + `actionHint`). **PATCH** `/decision-retrospectives/[id]` **status**(draft/reviewed/learned/archived)·outcome·`qualitySignals`·`whatWorked`/`whatDidNotWork`/`nextRule`. **POST** `/from-followup/[id]`·`/from-weekly-review`·`/from-today-candidate` 시드 생성(중복 시 기존 행); **`/from-today-candidate`**는 JSON 본문 길이 상한·candidate **화이트리스트**·관찰 요인 개수·요인 `message` 길이 검증(과대 시 **400** + `actionHint`, `detail_json`에 원문 candidate 전체 미저장). **`GET /api/decision-retrospectives/coach`** read-only 컨텍스트 미리보기(PB 호출 없음). **`POST /api/decision-retrospectives/coach`** PB 복기 **초안**만 생성(`qualityMeta.autoSaved: false`, `web_decision_retrospectives` **자동 insert 없음**). **판단 품질 복기**(수익률 평가·자동매매 아님). SQL 미적용 시 **503**·`decision_retrospective_table_missing`·`actionHint`.
  - `/api/trade-journal/*`
