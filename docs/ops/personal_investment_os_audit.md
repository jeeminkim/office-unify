...
| **끊긴 액션 UX** | 일부 “완료/확인”은 로컬 state만, `check_disclosure` 등 서버 액션 미렌더 — **EVO-028 1차**로 inbox detail/sourceRefs/steps·UI 보강(출처별 coverage 표: `docs/ops/action_items.md`). |
...
## 14. Thin Route Refactor - Today Brief Response Service Prep (2026-05-20, uncommitted)

### Audit summary

`apps/web/app/api/dashboard/today-brief/route.ts` still owns auth, Supabase access, portfolio data loading, quote loading, candidate generation, sector snapshot fallback, personalization summary, feedback application, US diagnostics, concentration/theme enrichment, qualityMeta assembly, bounded impression/snapshot writes, and final response shaping.

Already extracted server modules cover candidate generation, deck composition, score display, suitability, concentration, theme connection, feedback store, repeat exposure, decision trace, judgment quality, US diagnostics, and ops write budgeting.

Route-level work that can safely move first is request/query normalization. `todayBriefRouteRequest` is pure and does not touch DB or ops. `todayBriefResponseService` is only a skeleton for the next round; the full `buildTodayBriefResponse` extraction is intentionally not wired yet.

### Boundary decision

- Keep auth and `NextResponse` wrapper in the route.
- Keep existing bounded `today_candidate_impressions` and `today_candidate_snapshot` policy unchanged.
- Do not move candidate generation or qualityMeta assembly until contract tests cover the broad response shape.
- Preserve all existing API fields; any future service extraction must return `TodayBriefWithCandidatesResponse`.
## 2026-05-20 Update - Google Finance Direct Sheet Repair

- Added a local direct repair path for `portfolio_quotes`: dry-run by default, confirmed write only with `--confirm`, and `overwrite=false`.
- The UI apply route and CLI use the same repair core, keeping GET setup checks read-only.
- US anchor rows and GOOGLEFINANCE formulas are treated as data readiness blockers, not investment judgment.
- Post-check now makes the E2E state explicit: `anchorMatched`, `anchorOk`, formula pending count, and Today Brief gating reason.
- No SQL, no Supabase ledger mutation, and no automatic trading/order/rebalancing behavior were introduced.
