...
- **Follow-up 테이블 미적용·운영 집계:** `web_research_followup_items`가 없으면 followups·extract(save)·send-to-pb가 **503**·`code: research_followup_table_missing`·`actionHint`(SQL 경로 안내). `GET /api/dashboard/today-candidates/ops-summary` 응답에 **`usKrEmptyReasonHistogram`** 추가(`us_signal_candidates_empty`의 `detail.primaryReason` 가중 집계, 누락·구버전은 `unknown`). 홈 대시보드는 미국 신호 요약·진단을 상시 표시하고 원본 2열 후보는 `<details>` 접기.
