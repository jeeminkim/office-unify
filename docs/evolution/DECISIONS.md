...
- **결정:** 채택
- **이유:** 매일 생성되는 점검 메모가 자동 저장되면 복기 데이터가 사용자의 확인 기록이 아니라 시스템 노이즈가 된다.
- **대안:** Daily Review GET 시 자동 저장.
- **링크:** `web_daily_review_notes`, `POST /api/daily-review/notes`

### 2026-05-18 — PB 일일 메모는 deterministic note 이후 별도 단계로 분리한다

- **결정:** 채택
- **이유:** LLM 비용·출력 품질·저장 정책을 분리해야 한다. 1차는 deterministic 점검 메모와 명시 저장만 제공한다.
- **대안:** 매일 PB가 자동 생성·저장.
- **링크:** `/daily-review`

### 2026-05-18 — Action Item은 모든 출처에서 detail_json을 우선 채운다

- **결정:** 채택
- **이유:** 제목만 저장하면 사용자가 후속 행동을 알 수 없다.
- **대안:** 단순 inbox 유지.
- **링크:** `actionItemDetailBuilders.ts`

### 2026-05-18 — Daily Review는 읽기 전용 요약이지만 Action Item으로 연결된다

- **결정:** 채택
- **이유:** 매일 나온 진단을 읽고 끝내지 않고, 사용자가 선택한 항목만 작업 큐로 보낸다.
- **대안:** Daily Review에 자동 저장.
- **링크:** `/daily-review`, `/api/action-items`

### 2026-05-18 — 30일 복기 리포트는 수익률 평가가 아니라 판단 과정 평가다

- **결정:** 채택
- **이유:** 개인 투자 운영체제의 목적은 종목을 맞히는 것이 아니라 반복 실수를 줄이는 것이다.
- **대안:** 월간 수익률 리포트 중심.
- **링크:** `monthlyJudgmentReview.ts`, `/judgment-review`

### 2026-05-18 — 30일 복기 preview는 read-only, 저장은 명시 POST로 분리한다

- **결정:** 채택
- **이유:** 조회만으로 복기나 Action Item이 자동 저장되면 read-only 원칙과 충돌한다.
- **대안:** 매월 자동 저장.
- **링크:** `GET|POST /api/judgment-review/monthly`
