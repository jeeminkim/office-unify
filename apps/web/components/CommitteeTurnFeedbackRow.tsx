...
export function CommitteeTurnFeedbackRow(props: Props) {
  const { committeeTurnId, onSaved } = props;
  ...
  const submit = async (rating: PersonaChatFeedbackRating) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/committee/feedback", {
        method: "POST",
        headers: jsonHeaders,
  ...
  if (submitted) {
    return (
      <p className="mt-1 text-xs text-slate-500">
        평가 저장됨 ({ratingLabel(submitted)}) · 위원회 장기 기억에 반영되었습니다.
      </p>
    );
  }
  ...
  return (
    <div className="mt-2 rounded-md border border-slate-100 bg-slate-50/80 px-2 py-1.5">
      <div className="flex flex-wrap items-center gap-1.5">
        <span className="text-[11px] text-slate-500">이번 토론 기록이 도움이 되었나요?</span>
        <button
          type="button"
          disabled={loading}
