...
- **섹터 온도계** (`/sector-radar`): Sector Fear & Greed Radar — 관심 분야별 **한국 상장 ETF + (코인 등) US 티커** anchor를 기준으로 조정·과열을 점수화(판단 보조, 주문 실행 없음). `POST /api/sector-radar/refresh`로 `sector_radar_quotes` 시트(**2차 A–U**, `volume_avg`·`market`·`normalized_key` 포함)에 `GOOGLEFINANCE` 수식을 쓰고 read-back. `GET /api/sector-radar/watchlist-candidates`로 **관심종목 관찰 우선순위 큐**(매수 추천 아님)를 조회. 홈(`/`)에 Fear/Greed Top3 + **Crypto Radar** + **오늘의 관심종목 큐**. `/api/portfolio/summary`의 보유 행에 `sectorRadarBadge`(sector fear \| sector greed) optional. `/portfolio-ledger` 관심 목록에 레이더 배지. 선택 환경변수 `SECTOR_RADAR_QUOTES_SHEET_NAME`(기본 `sector_radar_quotes`).
