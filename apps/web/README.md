...
- **섹터 온도계** (`/sector-radar`): Sector Fear & Greed Radar — 관심 분야별 한국 상장 ETF anchor를 기준으로 조정·과열을 점수화(판단 보조, 주문 실행 없음). `POST /api/sector-radar/refresh`로 `sector_radar_quotes` 시트에 수식을 쓰고 read-back. 홈(`/`)에 조정 후보·과열 주의 Top3 요약. 선택 환경변수 `SECTOR_RADAR_QUOTES_SHEET_NAME`(기본 `sector_radar_quotes`).
